...
async function loadInitialData() {
  try { setData(await rpc('get_guide_initial_data', {})); }
  catch (error) { console.error(error); showToast(error.message || '데이터 불러오기 실패'); }
}
...
function setData(data) {
  ...
  renderAdminState(); renderAreaTabs(); renderSelectedAreaHero(); renderActivityList();
}
...
async function loginAdmin() {
  ...
  try {
    ...
    localStorage.setItem(ADMIN_KEY, code.trim()); APP.isAdmin = true;
    renderAdminState(); renderAreaTabs(); showToast('관리모드가 열렸습니다'); return true;
  } catch (error) { console.error(error); showToast(error.message || '관리모드 확인 실패'); return false; }
}

function logoutAdmin() { localStorage.removeItem(ADMIN_KEY); APP.isAdmin = false; renderAdminState(); renderAreaTabs(); showToast('관리모드를 종료했습니다'); }
function renderAdminState() { $('adminLoginBtn')?.classList.toggle('hidden', APP.isAdmin); $('adminLogoutBtn')?.classList.toggle('hidden', !APP.isAdmin); document.querySelectorAll('.admin-only').forEach(el => el.classList.toggle('hidden', !APP.isAdmin)); }
...
function renderAreaTabs() {
  ...
  box.innerHTML = areas.map(area => `<button class="area-block ${getAreaColorClass(area.name)}${String(area.id) === String(APP.currentAreaId) ? ' active' : ''}" type="button" data-area="${escAttr(area.id)}"><span>${esc(getAreaLabel(area.name))}</span>${APP.isAdmin ? `<button class="btn small" type="button" data-edit-area="${escAttr(area.id)}">수정</button>` : ''}</button>`).join('');
  box.querySelectorAll('[data-area]').forEach(button => button.addEventListener('click', event => { const edit = event.target.closest('[data-edit-area]'); if (edit) { event.stopPropagation(); openAreaModal(edit.dataset.editArea); return; } APP.currentAreaId = button.dataset.area; renderAreaTabs(); renderSelectedAreaHero(); renderActivityList(); }));
}

function renderSelectedAreaHero() { const area = currentArea(); const rows = currentRows(); $('selectedAreaDesc').textContent = area ? `${getAreaLabel(area.name)} 영역 프로그램 목록입니다.` : '영역을 클릭하면 프로그램 목록이 아래에 표시됩니다.'; $('selectedAreaCount').textContent = `프로그램 ${rows.length}개`; }
...
function renderActivityList() {
  ...
  box.innerHTML = rows.map((activity, index) => { const links = parseLinks(activity.links); const files = splitMaterials(activity.attachments); const desc = String(activity.description || activity.defaultMemo || '설명이 아직 없습니다.').trim(); return `<article class="activity-card" data-open="${escAttr(activity.id)}"><div><h3>${index + 1}. ${esc(activity.title || '이름 없는 프로그램')}</h3><p>${esc(desc)}</p><div class="activity-meta"><span class="meta-badge">링크 ${links.length}개</span><span class="meta-badge">PDF ${files.pdf.length}개</span><span class="meta-badge">그림 ${files.image.length}개</span></div></div><div class="activity-actions"><button class="btn light small" type="button" data-detail="${escAttr(activity.id)}">상세 보기</button><button class="btn primary small" type="button" data-primary="${escAttr(activity.id)}">바로 활용</button></div></article>`; }).join('');
...
function openActivityDetailById(id) {
  const found = APP.activities.find(row => String(row.id) === String(id)); if (!found) return;
  ...
  renderAdminState(); showModal('activityDetailModal');
}
...
function openPrimary(activity) {
  ...
  if (files.image.length) { openPrintView(activity, files.image); return; }
  if (files.pdf[0]) { window.open(getAttachmentOpenUrl(files.pdf[0]), '_blank'); return; }
...
function printImage(url) {
  ...
  win.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>활동자료 출력</title><style>html,body{margin:0;padding:0;background:#fff}img{display:block;max-width:100%;width:100%;height:auto;margin:0 auto}@media print{img{width:100%;max-height:100vh;object-fit:contain;page-break-after:always}}</style></head><body><img src="${escAttr(url)}" onload="setTimeout(function(){window.print()},250)"></body></html>`);
  win.document.close();
}

function renderLinks(raw) { const links = parseLinks(raw); if (!links.length) return ''; return `<div class="detail-section"><div class="detail-label">링크</div><div class="detail-link-list">${links.map(item => `<a href="${escAttr(item.url)}" target="_blank" rel="noopener" class="detail-link-item">${esc(item.label || item.url)}</a>`).join('')}</div></div>`; }
function renderMaterials(value) { const files = splitMaterials(value); const sections = []; if (files.pdf.length) sections.push(`<div class="detail-section"><div class="detail-label">PDF 설명자료</div><div class="detail-attach-list">${files.pdf.map(file => renderMaterialCard(file, 'PDF 설명자료')).join('')}</div></div>`); if (files.image.length) sections.push(`<div class="detail-section"><div class="detail-label">활동 그림자료</div><div class="image-preview-grid">${files.image.map(file => renderMaterialCard(file, '활동 그림자료')).join('')}</div></div>`); if (files.other.length) sections.push(`<div class="detail-section"><div class="detail-label">기타 자료</div><div class="detail-attach-list">${files.other.map(file => renderMaterialCard(file, '기타 자료')).join('')}</div></div>`); return sections.join(''); }
function renderMaterialCard(file, label) { const name = file.name || '파일'; const openUrl = getAttachmentOpenUrl(file); const downUrl = getAttachmentDownloadUrl(file); const previewUrl = getAttachmentPreviewUrl(file); let preview = '<div class="attach-preview-empty">미리보기를 지원하지 않는 파일입니다. 열기나 다운로드를 사용하세요.</div>'; if (isImageFile(file) && previewUrl) preview = `<img class="attach-preview-image" src="${escAttr(previewUrl)}" alt="${escAttr(name)}">`; else if (isPdfFile(file) && previewUrl) preview = `<iframe class="attach-preview-frame" src="${escAttr(previewUrl)}" loading="lazy"></iframe>`; return `<div class="attach-preview-box"><div class="attach-preview-head"><div><div class="attach-preview-name">${esc(name)}</div><div class="attach-preview-type">${esc(label)}</div></div><div class="attach-actions">${isImageFile(file) && openUrl ? `<button class="detail-attach-item" type="button" onclick="window.open('${escAttr(openUrl)}','_blank')">열기</button>` : openUrl ? `<a href="${escAttr(openUrl)}" target="_blank" rel="noopener" class="detail-attach-item">열기</a>` : ''}${downUrl ? `<a href="${escAttr(downUrl)}" target="_blank" rel="noopener" class="detail-attach-item" download>다운로드</a>` : ''}</div></div>${preview}</div>`; }

async function openAreaModal(areaId = '') { if (!APP.isAdmin && !(await loginAdmin())) return; APP.editingArea = null; $('areaModalTitle').textContent = '영역 추가'; $('areaNameInput').value = ''; $('areaSortInput').value = ''; if (areaId) { const area = APP.areas.find(row => String(row.id) === String(areaId)); if (area) { APP.editingArea = area; $('areaModalTitle').textContent = '영역 수정'; $('areaNameInput').value = area.name || ''; $('areaSortInput').value = String(area.sort || ''); } } showModal('areaModal'); }
async function saveArea() { if (!APP.isAdmin && !(await loginAdmin())) return; const data = { id: APP.editingArea ? APP.editingArea.id : '', name: $('areaNameInput').value.trim(), sort: $('areaSortInput').value.trim() }; if (!data.name) return showToast('영역 이름을 입력하세요'); await withButton($('saveAreaBtn'), '저장 중...', async () => { const res = await rpc('save_guide_area', { p_admin_code: getAdminCode(), p_area: data }); if (data.id) APP.currentAreaId = data.id; setData(res); hideModal('areaModal'); showToast('영역이 저장되었습니다'); }); }
...
async function openActivityModal(activityId = '') { if (!APP.isAdmin && !(await loginAdmin())) return; APP.editingActivity = null; $('activityModalTitle').textContent = '프로그램 등록'; ['activityTitleInput','activityDescriptionInput','activityMemoInput','activityLinksInput','activityPdfFilesInput','activityImageFilesInput','activitySortInput'].forEach(id => { const el = $(id); if (el) el.value = ''; }); $('activityColorSelect').value = 'blue'; $('activityExistingFiles').innerHTML = '첨부 없음'; renderAreaOptions(); $('activityAreaSelect').value = APP.currentAreaId || (($('activityAreaSelect').options[0] || {}).value || ''); if (activityId) { const found = APP.activities.find(row => String(row.id) === String(activityId)); if (found) { APP.editingActivity = found; $('activityModalTitle').textContent = '프로그램 수정'; $('activityAreaSelect').value = found.areaId || ''; $('activityTitleInput').value = found.title || ''; $('activityDescriptionInput').value = found.description || ''; $('activityMemoInput').value = found.defaultMemo || ''; $('activityLinksInput').value = found.links || ''; $('activitySortInput').value = String(found.sort || ''); $('activityColorSelect').value = found.color || 'blue'; $('activityExistingFiles').innerHTML = renderAttachmentNames(found.attachments); } } showModal('activityModal'); }
...
async function saveActivity() { if (!APP.isAdmin && !(await loginAdmin())) return; const areaId = $('activityAreaSelect').value; const title = $('activityTitleInput').value.trim(); if (!areaId) return showToast('영역을 선택하세요'); if (!title) return showToast('프로그램명을 입력하세요'); await withButton($('saveActivityBtn'), '저장 중...', async () => { showToast('자료를 업로드하고 있습니다'); const attachments = [...(APP.editingActivity ? normalizeAttachments(APP.editingActivity.attachments) : []), ...(await uploadFiles('activityPdfFilesInput', 'pdf')), ...(await uploadFiles('activityImageFilesInput', 'image'))]; const data = { id: APP.editingActivity ? APP.editingActivity.id : '', areaId, title, description: $('activityDescriptionInput').value.trim(), defaultMemo: $('activityMemoInput').value.trim(), links: $('activityLinksInput').value.trim(), color: $('activityColorSelect').value || 'blue', sort: $('activitySortInput').value.trim(), attachments }; const res = await rpc('save_guide_activity', { p_admin_code: getAdminCode(), p_activity: data }); APP.currentAreaId = areaId; setData(res); hideModal('activityModal'); showToast('프로그램이 저장되었습니다'); }); }
async function deleteCurrentActivity() { if (!APP.selectedActivity || !APP.isAdmin) return; if (!confirm('정말 이 프로그램을 삭제하시겠습니까?')) return; try { const res = await rpc('delete_guide_activity', { p_admin_code: getAdminCode(), p_activity_id: APP.selectedActivity.id }); APP.selectedActivity = null; setData(res); hideModal('activityDetailModal'); showToast('프로그램이 삭제되었습니다'); } catch (error) { console.error(error); showToast(error.message || '프로그램 삭제 실패'); } }
async function uploadFiles(inputId, kind) { const input = $(inputId); if (!input?.files?.length) return []; const uploaded = []; for (const file of Array.from(input.files)) { const ext = getFileExt(file.name) || 'file'; const path = `${kind}/${new Date().toISOString().slice(0, 10)}/${crypto.randomUUID()}.${ext}`; const res = await fetch(`${SUPABASE_URL}/storage/v1/object/${BUCKET}/${path}`, { method: 'POST', headers: { apikey: SUPABASE_ANON_KEY, Authorization: `Bearer ${SUPABASE_ANON_KEY}`, 'Content-Type': file.type || 'application/octet-stream', 'x-upsert': 'false' }, body: file }); if (!res.ok) { const data = await res.json().catch(() => ({})); throw new Error(data?.message || `${file.name} 업로드 실패`); } const url = `${SUPABASE_URL}/storage/v1/object/public/${BUCKET}/${path}`; uploaded.push(normalizeAttachment({ kind, name: file.name, mimeType: file.type || guessMimeType(file.name), size: file.size || 0, url, webViewLink: url, previewUrl: url, downloadUrl: url, directImageUrl: url })); } return uploaded; }
