...
.calendar-head{display:grid;grid-template-columns:64px 1fr 64px;align-items:center;gap:12px;margin-bottom:10px}.calendar-head h2{margin:0;text-align:center;font-size:30px}.calendar-head button{min-height:42px;padding:8px 12px;background:linear-gradient(180deg,#5886f4,#3264df);color:#fff;border:0;border-radius:12px;box-shadow:0 10px 20px rgb(48 92 210 / 18%);font-size:20px}.calendar-head button:last-child{justify-self:end}
...
.day-modal{position:fixed;inset:0;background:rgba(7,25,47,.45);display:grid;place-items:center;z-index:9999;padding:20px}.day-panel{width:min(420px,100%);background:#fff;border-radius:20px;padding:20px;box-shadow:0 20px 60px rgba(0,0,0,.2)}.day-panel h3{margin:0 0 12px;font-size:24px}.day-item{border:1px solid #d9e5f4;border-radius:14px;padding:12px;margin:8px 0}.day-item b{display:block;color:#07192f}.day-item small{display:block;margin-top:4px;color:#60738d}.day-panel button{width:100%;margin-top:10px;background:#3264df;color:#fff;border:0}
@media(max-width:720px){.calendar-card{padding:12px}.calendar-head{grid-template-columns:48px 1fr 48px;gap:6px}.calendar-head h2{font-size:22px}.calendar-head button{min-height:38px;font-size:18px;padding:6px}.calendar-week,.calendar-grid{gap:5px}.calendar-cell{min-height:68px;padding:7px;border-radius:10px}.calendar-cell strong{font-size:15px}.cal-dot{width:8px;height:8px}.calendar-cell small{font-size:11px}.calendar-week b{font-size:13px}}
`;
...
function openDayDetail(day) {
  const logs = (window.__records || []).filter(r => Number(d10(r.date).slice(-2)) === Number(day));
  const box = document.createElement('div');
  box.className = 'day-modal';
  box.innerHTML = `<div class="day-panel"><h3>${day}일 기록</h3>${logs.length ? logs.map(log => `<div class="day-item"><b>${titleOf(log)}</b><small>${noteOf(log) || '기록 내용 없음'}</small></div>`).join('') : '<p>기록 없음</p>'}<button data-close-day>닫기</button></div>`;
